...
var link_charge: int = 0            # the link entity's own reWelaCharge (BigCasterGolemLink: 1, +1 per 3 s, cap 3)
var link_charge_cap: int = 0
var link_charge_interval: int = 0   # its charge group's cooldown (TThinkImpulseTimerComponent + cooldown(False))
var link_charge_next_at: int = 0
var link_damage_mult: float = 1.0   # TModifierWelaDamageComponent.Multiply.ScaleWithResource(reWelaCharge): x (mult x charge)
