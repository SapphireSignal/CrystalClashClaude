...
## coGameplayShowDeckHotkeys (default false): DeckCard.dui shows the hotkey badge only with hud.ShowCardHotkeys.
var show_hotkeys := false
...
func _build_slot(commander: Commander, index: int) -> SlotView:
	...
	var hot_bg := HudStyle.picture(HudStyle.tex("HUD/DeckPanel/hotkey_background.png"), Rect2((SLOT_W - hot_w) / 2.0, slot_height - hot_h, hot_w, hot_h))
