...
- 2026-09-16 Simulation: fixed **30 Hz** tick, seeded per-match RNG, integer milliseconds for all timers
  (original used 32 ms variable-dt frames and unseeded random; player-invisible change, gives replays/tests).
...
- 2026-09-16 Sim is pure GDScript classes (no Nodes) in `game/sim/`; rendering nodes mirror sim state.
  Unit/card data are `.tres` Resources generated by `tools/` from the original `.ets` scripts.

## Status
Phase 1 done: repos cloned, `docs/original-architecture.md` + `docs/assets.md` written, Godot project boots empty.
Phase 2 in progress: core simulation (tick, entities, blackboard/eventbus-style stats, lanes, movement, combat, economy).
